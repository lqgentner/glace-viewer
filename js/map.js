...
/* One transparent pixel, PNG-encoded, answered for a tile the archive does not
 * store. The protocol returns `data: null` for those, and MapLibre's
 * RasterTileSource then leaves the tile in `loading` for ever: never drawn,
 * never retried, and `idle` can never fire. A blank tile lets it finish. Vector
 * archives already return an empty buffer, so they pass through untouched. See
 * "Missing tiles" in AGENTS.md. */
...
/* The float COGs the archives above were styled from, read and coloured in the
 * browser: one archive through a ramp, or two combined into false colour, both
 * through this one protocol — see js/cog-rgb.js.
 *
 * No control produces a `glace-rgb://` source any more; the tile-source switch
 * was retired in favour of the pre-styled archives once the comparison it
 * existed for was finished (see AGENTS.md). It stays registered because
 * registering is free — the reader it needs is not fetched until a tile is
 * asked for — and because that is what keeps the path a source spec away rather
 * than a rebuild away. */
...
const style = {
   ...
   * is reading a glacier. The page opens at z10, so that is the state a
